...
describe('createBurst', () => {
  test('emite exatamente count partículas na origem', () => {
  ...
  test('count = 0 retorna array vazio', () => {
    assert.deepEqual(createBurst(0, 0, '#fff', 0), []);
  });

  test('life inicial bate com maxLife (alpha começa em 1)', () => {
    const burst = createBurst(0, 0, '#0f0', 5, { rand: () => 0.3 });
    for (const p of burst) assert.equal(p.life, p.maxLife);
  });

  test('upwardBias subtrai vy (impulso vertical inicial pra cima)', () => {
  ...
  test('respeita ranges de size/speed/life', () => {
    ...
    for (const p of burst) {
      ...
      assert.ok(speed >= 10 - 1e-9 && speed <= 20 + 1e-9, `speed fora do range: ${speed}`);
...
describe('stepParticles', () => {
  test('avança posição por velocidade × dt', () => {
  ...
  test('aplica gravidade somando à vy', () => {
  ...
  test('decrementa life e remove partículas mortas', () => {
    const particles = [
